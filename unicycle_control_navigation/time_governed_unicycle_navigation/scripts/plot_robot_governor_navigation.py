...
class PlotTimeGovernedNavigation:
    """ 
    ...
    def plot_loop(self):  
        ...
        map_width = self.map_copy.width()
        map_height = self.map_copy.height()

        # Configuration Space Visualization
        safety_radius = self.robot_radius
        safety_grid_radius = safety_radius/resolution
        config_matrix = self.map_copy.occupancy_matrix().copy()
        D, Idx = self.map_copy.distance_field()
        D = D.flatten()
        config_idx = np.where( (D<=safety_grid_radius) & (D > 0))
        config_matrix = config_matrix.flatten()
        config_matrix[config_idx] = self.map_copy.occupied_threshold*0.9
        config_matrix = config_matrix.reshape(self.map_copy.occupancy_matrix().shape)
        ...
        plt.imshow(config_matrix, extent=[0, map_width, 0, map_height], cmap='binary', origin="lower")


        path_plot = ax.plot(self.path_copy[:,0], self.path_copy[:,1], 'r', linewidth=2)
        ...
        governor_path_plot[0].set_zorder(8)
        robot_path_plot[0].set_zorder(9)
...
if __name__ == '__main__':
    try:
        plot_domain = PlotTimeGovernedNavigation().start()    
